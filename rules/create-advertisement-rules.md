...
### R13. 投稿コピー（説明文 + ハッシュタグ）の生成

各動画につき、本番レンダリング成功後に `output/<product>/<date>/<id>.md` を 1 本生成する。SNS（X / Instagram 等）への投稿テキスト素材として使う。配信自体は本 Skill の責務外（R10）であり、ここでは「素材を `.md` で並べておく」ところまで。
...
- フロントマターは YAML、必須キーは `id` / `product` / `type` / `generated_at`
- 本文ブロックとハッシュタグ行の間は空行 1 つ
- ハッシュタグは半角スペース区切りで 1 行

#### 生成ルール

- **フック（最初の 1 文）**：30 文字前後（20〜45 文字）。句点で必ず終える。次のうち**最低 1 要素**を含めて視聴者の手を止める：
  - 具体的な数字（スコア / 日数 / 人数 / 倍率 等）
  - 意外性 / 逆説 / 「実は」「もう」「まだ」系の認知ズレ
  - 二人称（「あなた」「君」）への呼びかけ
- **本文**：2〜4 文、合計 200 文字以内を目安。`core.md` の「ブランドトーン」に従う（常体ベースの短文、断定 + 句読点リズム、テンションは商品ごとに合わせる）。動画の `variation_note` で言語化された差別化軸を 1 つは反映する。
- **ハッシュタグ**：**必ず 5 本**。以下の条件を満たす：
  - 商品名タグを 1 本含む（例：`#EvoArena`）
  - 残り 4 本は商品カテゴリ / 訴求軸 / ターゲット層に関連するもの
  - 日本語 / 英語混在 OK（読み手のターゲットに合わせる）
  - `core.md` の「避けたい表現」「競合実名」をハッシュタグにも持ち込まない

#### 入力情報

コピー生成時に参照するもの：

- `products/<name>/core.md`（ターゲット層 / 訴求の軸 / ブランドトーン / 避けたい表現）
- 当該 item の `variation_note`、`type`、`duration_sec`、`.tsx` のシーン構成

#### 失敗時の扱い

- 動画自体が R8 でスキップされた場合は `.md` も生成しない
- コピー生成だけが失敗した場合（モデルが空応答 / 規約違反のみ等）は当該 `.md` をスキップし、`issues.json` に `{ id, reason: "copy_generation_failed" }` を追記。動画ファイルとプレビューは残す
- manifest.json の当該 item で `copy` フィールドを省略する
...
8. 各動画と同階層に `<id>.md` が並んでおり、R13 のフォーマット（フロントマター / フック 1 文 / 本文 / ハッシュタグ 5 本）を満たしている
9. 各 `.md` のフックがハッシュタグや絵文字で始まらず、句点で終わる 1 文になっている
...
- ❌ `.md` のフックを `#hashtag` や絵文字、長文（45 文字超）で始める
- ❌ `.md` のハッシュタグを 5 本以外の本数にする / 改行で分ける / `core.md` の「避けたい表現」をタグにする
